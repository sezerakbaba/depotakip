...
export function renderKritik() {
  ...
  getAllItems().forEach(item=>{
    ...
    rows+=`<tr>
      ...
      <td>${durumBadge(d)}</td>
      <td class="td-mono u-100">${s.mevcut}</td>
      <td class="td-mono">${s.min}</td>
      <td class="td-mono u-101">${s.mevcut-s.min}</td>
    </tr>`;
  });
  ...
  if (_sktListe) {
    ...
    _sktListe.innerHTML = _urgent.length ? _urgent.map(i => {
      const mm = S.malzemeMeta[getKey(i.depo,i.ad)]||{};
      return `<div class="u-102">
        <div class="u-103">${i.ad}</div>
        <div>${window.sktBadge(mm.skt)}</div>
      </div>`;
    }).join('') : '<p class="u-104">Son 90 gün içinde biten kimyasal yok.</p>';
  }
  document.getElementById('kritik-tbody').innerHTML = rows ||
    '<tr><td class="u-105" colspan="7"><div class="empty-state"><div class="empty-icon"><i data-lucide="check-circle"></i></div><div class="empty-title">Kritik stok yok</div><div class="empty-desc">Tüm malzemeler yeterli seviyede.</div></div></td></tr>';
  ...
  document.getElementById('detay-content').innerHTML=`
    <div class="card u-26">
      <div style="background:${meta.color};padding:20px 24px;border-radius:10px 10px 0 0;display:flex;align-items:center;justify-content:space-between">
        ...
        <div class="u-107">
          <div class="u-108">
            <div class="u-109">${items.length}</div>
